...
import {
  ...
  Trash2, Edit3, Package, DollarSign, TrendingUp, AlertTriangle
} from 'lucide-react'
...
function CassaTab({ operatore }: { operatore: Operatore }) {
  ...
  const goPrec = () => setData(d => format(subDays(new Date(d), 1), 'yyyy-MM-dd'))
  const goOggi = () => setData(format(new Date(), 'yyyy-MM-dd'))
  const goSucc = () => setData(d => format(addDays(new Date(d), 1), 'yyyy-MM-dd'))
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden">
      ...
      <div className="px-4 lg:px-6 py-3 border-b border-white/5 flex-shrink-0">
        <div className="flex items-center justify-between gap-3 flex-wrap">
          <div className="flex items-center gap-2">
            <button onClick={goPrec} className="p-1.5 rounded-lg hover:bg-white/5 text-dac-gray-400"><ChevronLeft size={16} /></button>
            <button onClick={goOggi}
              className={`px-3 py-1.5 rounded-lg text-xs font-semibold ${isToday(new Date(data)) ? 'bg-dac-accent text-white' : 'bg-white/5 text-dac-gray-300'}`}>
              Oggi
            </button>
            <button onClick={goSucc} className="p-1.5 rounded-lg hover:bg-white/5 text-dac-gray-400"><ChevronRight size={16} /></button>
            <input type="date" value={data} onChange={e => setData(e.target.value)}
              className="bg-white/5 border border-white/10 rounded-lg px-3 py-1.5 text-xs text-white focus:outline-none" />
            <span className="text-sm font-display font-bold text-white ml-2">
              {format(new Date(data), 'EEEE d MMMM', { locale: it })}
            </span>
          </div>
          <button onClick={() => setShowForm(true)}
            className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-semibold bg-dac-accent text-white hover:opacity-90">
      ...
      <div className="flex-1 overflow-auto">
        ...
        : items.length === 0 ? <div className="text-center py-16 text-dac-gray-500 text-sm">Nessun movimento per questa data</div>
        : <div className="divide-y divide-white/[0.03]">
          {items.map(item => (
            <div key={item.id} className="flex items-center gap-4 px-4 lg:px-6 py-3 hover:bg-white/[0.03] transition-colors">
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-lg flex-shrink-0 ${item.tipo === 'Entrata' ? 'bg-dac-green/10' : 'bg-dac-red/10'}`}>
                {item.tipo === 'Entrata' ? '💰' : '📤'}
              </div>
              <div className="flex-1 min-w-0">
                <div className="text-sm font-semibold text-white">{item.tipo}</div>
                <div className="text-[10px] text-dac-gray-400">
                  {item.operatore_nome ?? '—'}{item.metodo ? ` • ${item.metodo}` : ''}{item.note ? ` • ${item.note}` : ''}
                </div>
              </div>
              <div className={`text-sm font-bold flex-shrink-0 ${item.tipo === 'Entrata' ? 'text-dac-green' : 'text-dac-red'}`}>
                {item.tipo === 'Entrata' ? '+' : '-'}€{Number(item.importo).toLocaleString('it-IT')}
              </div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm animate-fade-in" onClick={onClose}>
      <div className="bg-dac-card border border-white/10 rounded-2xl w-full max-w-sm mx-4 shadow-2xl animate-slide-up" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-5 py-4 border-b border-white/5">
          <h3 className="font-display font-bold text-white">💰 Registra Movimento</h3>
        ...
        <div className="p-5 space-y-3">
          ...
          <div className="flex rounded-xl bg-white/5 p-1">
            <button onClick={() => setTipo('Entrata')}
              className={`flex-1 py-2.5 rounded-lg text-xs font-bold transition-all ${tipo === 'Entrata' ? 'bg-dac-green text-white shadow-lg' : 'text-dac-gray-400'}`}>
              💰 Entrata
            </button>
            <button onClick={() => setTipo('Uscita')}
              className={`flex-1 py-2.5 rounded-lg text-xs font-bold transition-all ${tipo === 'Uscita' ? 'bg-dac-red text-white shadow-lg' : 'text-dac-gray-400'}`}>
              📤 Uscita
            </button>
          </div>
          <div>
            <label className="block text-[10px] font-semibold uppercase tracking-wider text-dac-gray-400 mb-1">Importo € *</label>
            <input type="number" value={importo || ''} onChange={e => setImporto(Number(e.target.value))} className="input-field text-center text-lg font-bold" min={0} step={0.01} placeholder="0.00" autoFocus />
          </div>
          <div>
            <label className="block text-[10px] font-semibold uppercase tracking-wider text-dac-gray-400 mb-1">Pagamento</label>
            <select value={metodo} onChange={e => setMetodo(e.target.value)} className="input-field">{METODI.map(m => <option key={m} value={m}>{m}</option>)}</select>
          </div>
          <div>
            <label className="block text-[10px] font-semibold uppercase tracking-wider text-dac-gray-400 mb-1">Note</label>
            <input type="text" value={note} onChange={e => setNote(e.target.value)} className="input-field" placeholder="Descrizione vendita..." />
          </div>
        </div>
...
function MagazzinoTab({ operatore }: { operatore: Operatore }) {
  ...
  function giorniScad(scadenza: string | null): number | null {
    if (!scadenza) return null
    return Math.floor((new Date(scadenza).getTime() - Date.now()) / 86400000)
  }
      ...
      <div className="px-4 lg:px-6 py-3 border-b border-white/5 flex-shrink-0">
        <div className="flex items-center justify-between gap-3 flex-wrap">
          <div className="grid grid-cols-3 gap-2 flex-1">
            <div className="px-3 py-2 rounded-xl bg-dac-accent/8 text-center">
              <div className="text-lg font-bold text-dac-accent">{totProdotti}</div>
              <div className="text-[8px] text-dac-gray-500 uppercase">Prodotti</div>
            </div>
            <div className="px-3 py-2 rounded-xl bg-dac-orange/8 text-center">
              <div className={`text-lg font-bold ${sottosoglia > 0 ? 'text-dac-orange' : 'text-dac-green'}`}>{sottosoglia}</div>
              <div className="text-[8px] text-dac-gray-500 uppercase">Sotto soglia</div>
            </div>
            <div className="px-3 py-2 rounded-xl bg-dac-green/8 text-center">
              <div className="text-lg font-bold text-dac-green">€{valoreTotale.toLocaleString('it-IT')}</div>
              <div className="text-[8px] text-dac-gray-500 uppercase">Valore stock</div>
            </div>
          </div>
          <button onClick={() => { setEditItem(null); setShowForm(true) }}
            className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-semibold bg-dac-accent text-white hover:opacity-90">
            <Plus size={14} /> Prodotto
          </button>
        </div>
        <div className="flex items-center gap-2 mt-3 flex-wrap">
          ...
          <select value={filterCat} onChange={e => setFilterCat(e.target.value)}
            ...
            {CATEGORIE_PROD.map(c => <option key={c} value={c}>{c}</option>)}
          </select>
        </div>
      </div>
        ...
        : <table className="w-full min-w-[700px]">
          <thead className="sticky top-0 z-10">
            <tr className="bg-dac-deep border-b border-white/10">
              ...
              <th className="th-cell text-center">Vendita</th>
          ...
          <tbody>
            {filtered.map(p => {
              ...
              const margine = (p.prezzo_vendita && p.prezzo_acquisto) ? ((p.prezzo_vendita - p.prezzo_acquisto) / p.prezzo_vendita * 100) : null
              const gg = giorniScad(p.scadenza)
              return (
                <tr key={p.id} className="border-b border-white/[0.03] hover:bg-white/[0.03] transition-colors">
                  ...
                  <td className="td-cell text-center">
                    {margine !== null ? (
                      <span className={`text-xs font-bold ${margine >= 30 ? 'text-dac-green' : margine >= 15 ? 'text-dac-orange' : 'text-dac-red'}`}>{margine.toFixed(0)}%</span>
                    ) : <span className="text-dac-gray-500 text-xs">—</span>}
                  </td>
                  <td className="td-cell">
                    {p.scadenza ? (
                      <div>
                        <span className="text-[10px] text-dac-gray-300">{format(new Date(p.scadenza), 'dd/MM/yy')}</span>
                        {gg !== null && <span className={`ml-1 text-[9px] font-bold ${gg < 0 ? 'text-dac-red' : gg < 30 ? 'text-dac-orange' : 'text-dac-gray-500'}`}>({gg}gg)</span>}
                      </div>
...
function ProdottoForm({ item, onClose, onSaved }: { item: ProdottoParafarmacia | null; onClose: () => void; onSaved: () => void }) {
  ...
  const margine = pVendita > 0 && pAcquisto > 0 ? ((pVendita - pAcquisto) / pVendita * 100) : 0
  ...
  async function salva() {
    ...
    const payload = {
      ...
      quantita, soglia_min: soglia,
      prezzo_acquisto: pAcquisto || null, prezzo_vendita: pVendita || null,
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div><label className="block text-[10px] font-semibold uppercase tracking-wider text-dac-gray-400 mb-1">Categoria</label>
              <select value={categoria} onChange={e => setCategoria(e.target.value)} className="input-field">{CATEGORIE_PROD.map(c => <option key={c} value={c}>{c}</option>)}</select>
            </div>
            <div><label className="block text-[10px] font-semibold uppercase tracking-wider text-dac-gray-400 mb-1">Marca</label>
              <input type="text" value={marca} onChange={e => setMarca(e.target.value)} className="input-field" />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div><label className="block text-[10px] font-semibold uppercase tracking-wider text-dac-gray-400 mb-1">Quantità</label>
              <input type="number" value={quantita} onChange={e => setQuantita(Number(e.target.value))} className="input-field text-center" min={0} />
            </div>
            <div><label className="block text-[10px] font-semibold uppercase tracking-wider text-dac-gray-400 mb-1">Soglia min.</label>
              <input type="number" value={soglia} onChange={e => setSoglia(Number(e.target.value))} className="input-field text-center" min={0} />
            </div>
          </div>
          {/* Prezzi con margine live */}
          <div className="p-3 rounded-xl bg-white/3 border border-white/5">
            <div className="text-[10px] font-semibold uppercase tracking-wider text-dac-gray-400 mb-2">💰 Prezzi</div>
            <div className="grid grid-cols-3 gap-2">
              <div><label className="block text-[9px] text-dac-gray-500 mb-1">Acquisto €</label>
                <input type="number" value={pAcquisto || ''} onChange={e => setPAcquisto(Number(e.target.value))} className="input-field text-center" min={0} step={0.01} />
              </div>
              <div><label className="block text-[9px] text-dac-gray-500 mb-1">Vendita €</label>
                <input type="number" value={pVendita || ''} onChange={e => setPVendita(Number(e.target.value))} className="input-field text-center" min={0} step={0.01} />
              </div>
              <div><label className="block text-[9px] text-dac-gray-500 mb-1">Margine</label>
                <div className={`input-field text-center font-bold ${margine >= 30 ? 'text-dac-green' : margine >= 15 ? 'text-dac-orange' : 'text-dac-red'}`}>
                  {margine > 0 ? `${margine.toFixed(0)}%` : '—'}
                </div>
              </div>
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            ...
            <div><label className="block text-[10px] font-semibold uppercase tracking-wider text-dac-gray-400 mb-1">Fornitore</label>
              <input type="text" value={fornitore} onChange={e => setFornitore(e.target.value)} className="input-field" />
            </div>
          </div>
